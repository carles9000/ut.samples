	...
	TEXT TO cJs
      ...
      if (navigator.share) {
          ...
          const shareData = {
              title: "ElTitulo",
              text: "ElTexto",
              url: "https://rxbase.duckdns.org/images/mercury_mini.png",
          };

          // Si deseas compartir una imagen, puedes hacerlo como un Blob
          // Aquí se utiliza una imagen de ejemplo como base64
          fetch('https://rxbase.duckdns.org/images/mercury_mini.png')
              .then(response => response.blob())
              .then(imageBlob => {
                  shareData.files = [new File([imageBlob], 'mercury_mini.png', { type: 'image/png' })];
